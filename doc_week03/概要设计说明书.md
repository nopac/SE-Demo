...
* 内部接口
    ...
    * 软件接口<br>
      ...
      | createCode               |        |                        |                                       |                       | 生成验证码                                                                                 |
      |                          |        |                        |                                       |                       |                                                                                       |
      | **UserEXMController**        |        |      **/exmU**                  |                                       |                       |                                                                                       |
      | changeStatus             | put    | /exmU/{status}         | Users users,status                    | 判断并更新是否通过审核           | 传入的status=1则通过审核，status=0则不通过                                                         |
      | findPage                 | delete | /exmU                  | pageNumber, pageSize, searchText,type | 根据type显示对应模糊查询        | 默认为gname,可判断为uname,uid,status                                                         |
      |                          |        |                        |                                       |                       |                                                                                       |
      ...
      |                          |        |                        |                                       |                       |                                                                                       |
      | **GoodsEXMController**       |        |  **/exmG**                      |                                       |                       |                                                                                       |
      | agree                    | put    | /exmG/{status}         | Goods goods,status                    | 判断并更新是否通过审核           | 传入的status=1则通过审核，status=0则不通过                                                         |
      | findPage                 | delete | /exmG                  | pageNumber, pageSize, searchText,type | 根据type显示对应模糊查询        | 默认为gname,可判断为uname,uid,status                                                         |
      |                          |        |                        |                                       |                       |                                                                                       |
      ...
      | received                 | put    |                        |                                       |                       | 确认收货需要修改内容：1 订单状态修改，成交时间增加 2：24小时候成交，不可退货；用户积分增加；商品销量增加；商家营业额增加，总交易额增加；平台判断商家等级，收取手续费 |
      | findPage                 | get    | /orders                | pageNumber, pageSize, searchText      | 根据当前页、每页条数和查询内容进行分页查询 |                                                                                       |
      |                          |        |                        |                                       |                       |                                                                                       |
      | **GoodsEXMController**       |        | **/exmO**                       |                                       |                       |                                                                                       |
      | out                      | put    | /exmO/{status}         | Orders orders,status                  | 判断并更新是否通过审核           | 传入的status=1则通过发货，status=-1则通过退货。发货需要修改：商品库存减少；通过退货需要修改订单状态，退货时间                       |
      | getGoods                 | put    | /exmO/getGoods         | Orders orders                         | 根据type显示对应模糊查询        | 默认为gname,可判断为uname,uid,status                                                         |
      | reject                   | delete | /exmO/{status}         | Orders orders                         | 拒绝退货申请                | 修改status为-3                                                                           |
      | delete                   | delete | /exmO                  | Orders orders                         | 根据获取传来的orders的oid删除   | 订单什么时候会被删除？不需要此函数                                                                     |
      | judgeStatus              |        |                        |                                       |                       | 判断此时是否能退货                                                                             |
      | findPage                 | get    | /exmO                  | pageNumber, pageSize, searchText,type | 根据type显示对应模糊查询        | 默认为空,可判断为sent,back,uid,gid                                                            |
      |                          |        |                        |                                       |                       |                                                                                       |
      | **CartController**           |        |  **/cart**                      |                                       |                       |                                                                                       |
