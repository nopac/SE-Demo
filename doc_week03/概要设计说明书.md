...
|数据类型 | 名称 |属性名| 约束 |  备注   |   
|-----|-----|-----|---|----|
|String|管理员编号|AdmID|非空|
|String|管理员密码|password|
...
|数据类型 | 名称 |属性名| 约束 |  备注   |
  |-----|-----|-----|---|----|
|int|订单编号|Oid|非空|
|int|用户编号|Uid|非空|买家的Uid|
|int|商品编号|Gid|非空|
|int|商家编号|Mid|非空|商家的Uid|
|int|评价|Rid|
|DATETIME|提交时间|Subtime|非空|
|DATETIME|成交时间|SubSucTime|
|DATETIME|发货时间|SentTime|
|double|单价|Price|非空|
|DATETIME|退货时间|RetTime|
|DATETIME|退货成功时间|RetSecTime|
|int|状态|status| | 0-待发货，1-已发货，2-已收货，3-交易成功，-1-退货中，-2退货成功，-3退货失败|
|double|手续费|Commission|
|String|地址|Address|
|int|数量|Number|
|String|快递单号|DeliverID| | 发货的时候弹窗填|
|double|总金额|sum|
|String|商品名|Gname|
|String|商家备注|MNote|
...
|String|文字|Remark|
|DATETIME|评价时间|Time|
|String|评价者|Fromid| | 评价者Uid|
|String|被评价者|Toid| |被评价者Uid|
|double|被评价者满意度|Rate| | 评价完后被评价者满意度|
|int|类型|type| |1-用户对商家，2-用户对商品，3-商家对用户|
|int|订单|oid| | 订单编号
